...
const PAGE_SIZE = 30;
const state = { query:"", category:"전체", view:"all", favoriteOnly:false, limit:PAGE_SIZE, favorites:new Set(JSON.parse(localStorage.getItem(FAVORITE_KEY)||"[]")) };
...
function cardHtml(product){
  ...
  return `<article class="product-card" data-id="${html(product.id)}">
    ...
    <div class="card-body">
      ...
      <details class="details"><summary>상세 보기</summary><span class="section-label">제품 상세</span><p>${html(product.product_detail)}</p><span class="section-label">확인된 사양/기능</span><ul>${specs}</ul><span class="section-label">현장 확인 포인트</span><ul>${points}</ul><span class="section-label">출처 URL</span><ul class="source-list">${sources}</ul></details>
    </div>
  </article>`;
}
